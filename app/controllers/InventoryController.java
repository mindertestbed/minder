...
public class InventoryController extends Controller {
	...
	public static Result editRunConfigurationForm(Long id) {
		...
		for (MappedWrapper mappedWrapper : rc.mappedWrappers) {
			rome.mappedWrappers.add(new MappedWrapperModel(mappedWrapper.id,
					mappedWrapper.parameter.name, mappedWrapper.wrapper.name));
		}
	...
	public static Result doEditRunConfiguration() {
		Form<RunConfigurationEditorModel> frm = RUN_CONFIGURATION_FORM
				.bindFromRequest();
	...
	public static Result doRunRunconfiguration(Long id) {
		/*
		 * if(id not startet){ run(id) return ok(runningRIvetLister.render(id))
		 * }
		 * 
		 * if(id not finished){ return ok(runningRIvetLister.render(id)) }
		 */
